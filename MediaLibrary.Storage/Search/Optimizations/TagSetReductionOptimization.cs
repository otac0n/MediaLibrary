...
namespace MediaLibrary.Storage.Search.Optimizations
{
    ...
    public sealed class TagSetReductionOptimization : Optimization
    {
        public override Expression Replace(DisjunctionExpression expression)
        {
            ...
            if (tagSetCount >= 2)
            {
                var reducedSet = lookup[true].Cast<NegationExpression>()
                    .OrderByDescending(n => ((TagExpression)n.Expression).Tags.Count)
                    .Aggregate(
                        ImmutableList<NegationExpression>.Empty,
                        (l, e) => !l.Any(t => ((TagExpression)t.Expression).Tags.IsSupersetOf(((TagExpression)e.Expression).Tags)) ? l.Add(e) : l);
        ...
        public override Expression Replace(ConjunctionExpression expression)
        {
            ...
            if (tagSetCount >= 2)
            {
                var reducedSet = lookup[true].Cast<TagExpression>()
                    .OrderBy(t => t.Tags.Count)
                    .Aggregate(
                        ImmutableList<TagExpression>.Empty,
